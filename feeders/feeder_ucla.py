...
class Feeder(Dataset):
    ...
    def load_data(self):
        ...
        for data in self.data_dict:
            ...
            with open(self.nw_ucla_root + file_name + ".json") as f:
    ...
    def rand_view_transform(self, X, agx, agy, s):
        ...
        Rx = np.asarray(
            [[1, 0, 0], [0, math.cos(agx), math.sin(agx)], [0, -math.sin(agx), math.cos(agx)]]
        )
        Ry = np.asarray(
            [[math.cos(agy), 0, -math.sin(agy)], [0, 1, 0], [math.sin(agy), 0, math.cos(agy)]]
        )
        ...
        if "b" in self.data_type:
            ...
            for bone_idx in range(20):
                data_bone[:, self.bone[bone_idx][0] - 1, :] = (
                    data[:, self.bone[bone_idx][0] - 1, :] - data[:, self.bone[bone_idx][1] - 1, :]
                )
            data = data_bone
