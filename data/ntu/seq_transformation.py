...
def remove_nan_frames(ske_name, ske_joints, nan_logger):
    ...
    for f in range(num_frames):
        if not np.any(np.isnan(ske_joints[f])):
            ...
            nan_logger.info(f"{ske_name}\t{f + 1:^5}\t{nan_indices}")
    ...
    for idx, ske_joints in enumerate(skes_joints):
        ...
        for f in range(num_frames):
            ...
            if (ske_joints[f, 75:] == 0).all():
                ske_joints[f, :75] = (ske_joints[f, :75] - np.tile(origin, 25)) / dist[f] + np.tile(
                    origin, 25
                )
            else:
                ske_joints[f] = (ske_joints[f] - np.tile(origin, 50)) / dist[f] + np.tile(
                    origin, 50
                )
