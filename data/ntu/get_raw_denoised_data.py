...
def denoising_by_length(ske_name, bodies_data):
    ...
    for bodyID, body_data in new_bodies_data.items():
        ...
        if length <= noise_len_thres:
            ...
            noise_len_logger.info(
                "{}\t{}\t{:.6f}\t{:^6d}".format(ske_name, bodyID, body_data["motion"], length)
            )
...
def denoising_by_motion(ske_name, bodies_data, bodies_motion):
    ...
    for bodyID, motion in bodies_motion[1:]:
        if (motion < noise_mot_thres_lo) or (motion > noise_mot_thres_hi):
            ...
            noise_mot_logger.info(f"{ske_name}\t{bodyID}\t{motion:.6f}")
        else:
...
def remove_missing_frames(ske_name, joints, colors):
    ...
    if num_bodies == 2:  # DEBUG
        ...
        if max(cnt1, cnt2) > 0:
            if cnt1 > cnt2:
                info = f"{ske_name}\t{num_frames:^10d}\t{cnt1:^6d}\t{cnt2:^6d}\t{start:^5d}\t{end:^3d}"
                missing_skes_logger1.info(info)
            else:
                info = f"{ske_name}\t{num_frames:^10d}\t{cnt1:^6d}\t{cnt2:^6d}"
    ...
    if num_missing > 0:  # Update joints and colors
        ...
        missing_count += 1
        missing_skes_logger.info(f"{ske_name}\t{num_frames:^10d}\t{num_missing:^11d}")
...
def get_raw_denoised_data():
    ...
    for idx, bodies_data in enumerate(raw_skes_data):
        ...
        if (idx + 1) % 1000 == 0:
            print(
                "Processed: %.2f%% (%d / %d), " % (100.0 * (idx + 1) / num_skes, idx + 1, num_skes)
    ...
    print(
        f"Saved raw denoised positions of {np.sum(frames_cnt)} frames into {raw_skes_joints_pkl}"
    )
