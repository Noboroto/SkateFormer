...
def get_raw_bodies_data(skes_path, ske_name, frames_drop_skes, frames_drop_logger):
    ...
    assert num_frames_drop < num_frames, "Error: All frames data (%d) of %s is missing or lost" % (
        num_frames,
        ske_name,
    )
    if num_frames_drop > 0:
        frames_drop_skes[ske_name] = np.array(frames_drop, dtype=np.int)
        frames_drop_logger.info(
            f"{ske_name}: {num_frames_drop} frames missed: {frames_drop}\n"
        )
...
def get_raw_skes_data():
    ...
    for idx, ske_name in enumerate(skes_name):
        ...
        if (idx + 1) % 1000 == 0:
            print(
                "Processed: %.2f%% (%d / %d)" % (100.0 * (idx + 1) / num_files, idx + 1, num_files)
            )
